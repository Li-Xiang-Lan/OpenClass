...
public class RootFrameLayout extends FrameLayout {
    ...
    private void errorViewAddData(int iconImage, String textTip) {
        ...
        if (iconImageView != null &&iconImage!=0&& iconImageView instanceof ImageView) {
    ...
    private void showHideViewById(int id) {
        for (int i = 0; i < layoutSparseArray.size(); i++) {
            ...
            if(key == id) {
                ...
                View view = layoutSparseArray.get(id);
                switch (id){
                    case LAYOUT_NETWORK_ERROR_ID:  //网络出错
                        setImageResource(view, R.id.network_img,R.drawable.network_error);
                        break;
                    case LAYOUT_ERROR_ID: //加载出错
                        setImageResource(view,R.id.error_img,R.drawable.error);
                        break;
                    case LAYOUT_EMPTYDATA_ID: //空数据
                        setImageResource(view,R.id.empty_img,R.drawable.nodata);
                        break;
                }
    ...
    private void setImageResource(View view,int id,int imageId){
        ImageView imageView = view.findViewById(id);
        LoadLocalImageUtil.getInstance().displayFromDrawable(imageId,imageView);
    }
